...
export const uploadvideo = async (req, res) => {
  ...
  const customThumbnailUrl = thumbnailFile
    ? getPublicUploadPath(thumbnailFile.path)
    : thumbnailUrl;

  let autoGeneratedThumbnailUrl = "";
  if (!customThumbnailUrl) {
    try {
      autoGeneratedThumbnailUrl = await generateAutoThumbnail(videoFile.path);
    } catch (error) {
  ...
  try {
    const storedVideoPath = getPublicUploadPath(videoFile.path);
